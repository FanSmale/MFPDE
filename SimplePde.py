...
Machine learning approximation algorithms for high-dimensional fully nonlinear partial differential
 equations and second-order backward stochastic dierential equations
"""
...
np.savetxt(str(globalname) + "_d" + str(d) + "_" +
           #datetime.datetime.now().strftime('%Y-%m-%d-%H:%M:%S') + ".csv", output,
           datetime.datetime.now().strftime('%Y-%m-%d') + ".csv", output,
           delimiter=",", header="step , loss function , Y0 , learning rate , running time ")
